// ============================================================
// TABLET CONTROL - OSU (Open Smash Ultimate) - v1.0.0
// Archivo exclusivo para OSU. Clonado de AFK. No tocar para Córdoba ni Mendoza.
// Stages Game 1: Battlefield, Smashville, Town and City, Small Battlefield, Pokemon Stadium 2
// v1.0.0 - Clon de TabletControlAfk con branding y paleta OSU
// ============================================================
    ...
    <div style={{
      ...
      <div>
        ...
        <p style={{ margin: 0, fontSize: 15, color: 'rgba(255,255,255,0.65)', lineHeight: 1.5, textShadow: '1px 1px 4px rgba(0,0,0,0.8)' }}>
          <span style={{ color: '#C9A6E8', fontWeight: 800 }}>{turnPlayerName}</span>{' está '}{action}
        </p>
      </div>
    </div>
  );
...
// ── Stages hardcodeados para Game 1 de OSU ─────────────────
const GAME1_STAGES_OSU = [
...
// ─────────────────────────────────────────────────────────────
export default function TabletControlOsu({ sessionId, playerName, playerIndex, matchToken }) {
  ...
  // Solo el setup de stream (osu-stream) y tablet (osu-tablet) envían datos al overlay True Combo
  const prevScoreRef = useRef({ p1: null, p2: null, tournament: null });
  useEffect(() => {
    if (!session || session.phase === 'IDLE' || session.phase === 'CHECKIN') return;
    if (sessionId !== 'osu-stream' && sessionId !== 'osu-tablet') return;
  ...
  // Sincronizar stage seleccionado con overlay player-intro (solo osu-stream y osu-tablet)
  useEffect(() => {
    if (!session || (sessionId !== 'osu-stream' && sessionId !== 'osu-tablet')) return;
  ...
  // Activar automáticamente modo 1 dispositivo para osu-tablet
  useEffect(() => {
    if (sessionId === 'osu-tablet' && session && session.phase === 'CHECKIN' && !session.singleDeviceMode) {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          <img src="/images/OSU.png" alt="OSU" className="w-24 h-24 mx-auto mb-4 animate-pulse" />
  ...
  const confirmAction = () => {
    ...
    switch (pendingAction.type) {
      ...
      case 'character':
        ...
        if ((sessionId === 'osu-stream' || sessionId === 'osu-tablet') && pendingAction.characterId && pendingAction.characterId !== 'random' && !pendingAction.isRandom) {
      ...
      <div className="sticky top-0 z-40 bg-black/95 backdrop-blur-md px-3 pt-2 pb-2 sm:px-4 sm:pt-3 sm:pb-2 border-b border-white/20 shadow-xl overflow-hidden">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between gap-2">
              <img src="/images/OSU.png" alt="OSU" className="h-7 sm:h-9 w-auto object-contain flex-shrink-0" onError={(e) => { e.target.style.display = 'none'; }} />
              {session.round ? (
            ...
            {myPlayer ? (
              ...
              <div style={{ textAlign: 'center', padding: '8px 0 16px' }}>
                ...
                <p style={{ margin: '0 0 28px', fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
                  Solo <strong style={{ color: '#C9A6E8' }}>{session.player1?.name}</strong> y <strong style={{ color: '#C9A6E8' }}>{session.player2?.name}</strong> pueden hacer check-in aquí.
                </p>
                <a
                  href="/home"
                  style={{
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 sm:p-4 shadow-xl border border-white/20 flex flex-col relative">
            ...
            <div className="flex flex-col gap-1.5 sm:gap-2 pb-2">
              {session.currentGame === 1 ? (
                /* Game 1 - 5 stages fijos de OSU: 3+2 */
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2">
                    {GAME1_STAGES_OSU.slice(0, 3).map(s => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-6 gap-1.5 sm:gap-2">
                    ...
                    {GAME1_STAGES_OSU.slice(3, 5).map(s => (
