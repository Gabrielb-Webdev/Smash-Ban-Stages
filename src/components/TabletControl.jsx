...
export default function TabletControl({ sessionId, playerName, playerIndex, matchToken }) {
  const { session, sessionError, selectRPSWinner, banStage, selectStage, selectCharacter, setGameWinner, proposeGameWinner, rejectGameWinner, playerCheckin, getPlayerHistory, requestMatchDelay, playerUnavailable, enableSingleDevice, rpsPick } = useWebSocket(sessionId);
  ...
  useEffect(() => {
    ...
    if (session.currentGame >= 2 && session.phase === 'CHARACTER_SELECT') {
      ...
      if (session.currentTurn === 'player1' && 
          ...
          (previousCharacters.player1 || session.lastCharacters?.player1) &&
      ...
      if (session.currentTurn === 'player2' && 
          ...
          !session.player2.character && 
          (previousCharacters.player2 || session.lastCharacters?.player2) &&
  ...
  const handleSelectCharacter = (characterId) => {
    ...
    if (getSkinCount(characterId) <= 1) {
      setPendingAction({ type: 'character', characterId, characterName: character.name, characterImage: getStockIconPath(characterId, 1), player: targetPlayer, skin: 1 });
    } else {
      setSkinModal({ characterId, characterName: character.name, player: targetPlayer });
    }
  };
  ...
  const handleRandomCharacter = () => {
    ...
    if (!targetPlayer) return;
    setPendingAction({ type: 'character', characterId: 'random', characterName: '?', characterImage: null, player: targetPlayer, isRandom: true });
  };
  ...
  return (
    ...
    >
      ...
      >
          ...
          {isMendoza() && (
            <div className="mt-4 flex items-center justify-center gap-4 opacity-90">
              <span className="text-white/80 text-2xl sm:text-4xl font-bold tracking-wide">
                POWERED BY:
              </span>
              <img 
      ...
      <div className="p-3 md:p-4 max-w-7xl mx-auto flex flex-col gap-3 md:gap-4 pb-24">
        ...
        {/* RPS Phase - Piedra Papel o Tijera */}
        {session.phase === 'RPS' && (
            ...
            <div className="flex-1 flex flex-col gap-1.5 sm:gap-2 overflow-y-auto pb-2">
              ...
              {(isMendoza() || isWarui) ? (
                ...
                session.currentGame === 1 ? (
                  ...
                  /* Games 2-5 para Mendoza - Layout dinámico de 8 stages (3+3+2) */
                  <>
                    {/* Primera fila: 3 stages */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2">
                ...
                /* Córdoba: Layout hardcodeado solo para Game 1, dinámico para Games 2+ */
                session.currentGame === 1 ? (
                  /* Game 1 para Córdoba - Layout hardcodeado de 5 stages */
                  <>
                    {/* Primera fila: 3 stages */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2">
                      <button 
          ...
          isWarui ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 10, minHeight: 'calc(100dvh - 72px)', paddingTop: '1rem', paddingBottom: '5rem' }}>
              ...
              <div className="flex items-center justify-center gap-3">
                <div style={{ background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', borderRadius: 12, padding: '4px 18px', border: '2px solid rgba(168,85,247,0.6)' }}>
                  <span style={{ fontFamily: 'Anton', fontSize: 13, color: '#fff', letterSpacing: 2 }}>GAME {session.currentGame}</span>
                </div>
                <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.15), rgba(255,255,255,0.05))', borderRadius: 12, padding: '4px 18px', border: '2px solid rgba(255,255,255,0.2)' }}>
                  <span style={{ fontFamily: 'Anton', fontSize: 13, color: '#E8A000', letterSpacing: 1 }}>EN COMBATE</span>
                </div>
              </div>

              {/* Stage - imagen prominent */}
              {session.selectedStage && (
                <div style={{ position: 'relative', borderRadius: 14, overflow: 'hidden', border: '2px solid rgba(168,85,247,0.5)', flexShrink: 0, height: 72 }}>
                  <img
                    src={getStageData(session.selectedStage)?.image}
                    alt={getStageData(session.selectedStage)?.name}
                    style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
                  />
                  <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.2) 60%, transparent 100%)' }} />
                  <div style={{ position: 'absolute', bottom: 6, left: 0, right: 0, textAlign: 'center' }}>
                    <span style={{ fontFamily: 'Anton', fontSize: 15, color: '#fff', letterSpacing: 1, textShadow: '2px 2px 6px rgba(0,0,0,1)', textTransform: 'uppercase' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'stretch' }}>
                ...
                <button
                  ...
                  {session.player1.character ? (
                    <img
                      src={getStockIconPath(session.player1.character, session.player1.skin || 1)}
                      alt={getCharacterData(session.player1.character)?.name}
                      style={{ width: 52, height: 52, objectFit: 'contain', filter: 'drop-shadow(0 2px 6px rgba(0,0,0,0.8))' }}
                      onError={e => { e.target.style.display = 'none'; }}
                    />
                  ) : (
                    <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>❓</div>
                  )}
                  {/* Name */}
                  <p style={{ margin: 0, fontFamily: 'Anton', fontSize: 13, color: '#fff', textAlign: 'center', textShadow: '2px 2px 6px rgba(0,0,0,0.9)', lineHeight: 1.2, wordBreak: 'break-word', maxWidth: '100%' }}>
                    {session.player1.name}
                  </p>
                  {/* Character name */}
                ...
                <button
                  ...
                  {session.player2.character ? (
                    <img
                      src={getStockIconPath(session.player2.character, session.player2.skin || 1)}
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      ...
                      <button
                        ...
                        style={{ padding: '14px 8px', borderRadius: 14, border: '2px solid rgba(220,38,38,0.6)', background: 'linear-gradient(135deg, rgba(220,38,38,0.35), rgba(185,28,28,0.2))', color: '#fff', fontFamily: 'Anton', fontSize: 15, cursor: 'pointer', letterSpacing: 1 }}
                      >❌ RECHAZAR</button>
                    </div>
                  </div>
                ) : (
            ...
            <div className="bg-gradient-to-br from-smash-red/90 to-red-700/90 rounded-2xl p-8 shadow-2xl border-4 border-white max-w-lg w-full animate-scale-in">
              <div className="text-center mb-6">
                ...
                <div className="bg-black/40 rounded-xl p-6 border-2 border-white/30">
                  <img 
                    src={getCharacterData(previousCharacters.player1 || session?.lastCharacters?.player1)?.image} 
                    alt={getCharacterData(previousCharacters.player1 || session?.lastCharacters?.player1)?.name}
                  ...
                  <p className="text-white text-2xl font-bold"
                     style={{ textShadow: '2px 2px 4px rgba(0, 0, 0, 0.8)' }}>
                    {getCharacterData(previousCharacters.player1 || session?.lastCharacters?.player1)?.name}
                  </p>
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-smash-blue/90 to-blue-700/90 rounded-2xl p-8 shadow-2xl border-4 border-white max-w-lg w-full animate-scale-in">
              <div className="text-center mb-6">
                ...
                <div className="bg-black/40 rounded-xl p-6 border-2 border-white/30">
                  <img 
                    src={getCharacterData(previousCharacters.player2 || session?.lastCharacters?.player2)?.image} 
                    alt={getCharacterData(previousCharacters.player2 || session?.lastCharacters?.player2)?.name}
                    className="w-32 h-32 mx-auto mb-3 rounded-full border-4 border-white shadow-2xl"
                    onError={(e) => {
                      e.target.style.display = 'none';
                      e.target.nextSibling.style.display = 'block';
                  ...
                  <p className="text-white text-2xl font-bold"
                     style={{ textShadow: '2px 2px 4px rgba(0, 0, 0, 0.8)' }}>
                    {getCharacterData(previousCharacters.player2 || session?.lastCharacters?.player2)?.name}
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
        ...
        {skinModal && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-black to-smash-purple rounded-2xl p-4 shadow-2xl border-4 border-smash-yellow max-w-sm w-full">
              ...
              <button onClick={() => { setSkinModal(null); }} className="w-full py-2 bg-gray-700 text-white font-bold text-sm rounded-lg active:scale-95 touch-manipulation">
                ❌ Cancelar
              </button>
            </div>
          </div>
          ...
          >
            <div
              ...
              style={{
                ...
                animation: 'modalPopTC 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) both',
              }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="absolute top-0 left-0 right-0" style={{ height: '3px', background: `linear-gradient(90deg, transparent, ${turnModal.accent}, transparent)`, animation: 'accentPulseTC 2s ease-in-out infinite' }} />
              <div className="px-8 pt-10 pb-8">
                <div className="mx-auto mb-5 flex items-center justify-center text-5xl" style={{ width: 90, height: 90, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', border: `2px solid ${turnModal.accent}66`, boxShadow: `0 0 24px ${turnModal.accent}44`, animation: 'fadeSlideUpTC 0.35s ease both 0.05s' }}>
                  {turnModal.icon}
                </div>
                <p className="font-bold uppercase" style={{ color: turnModal.accent, fontSize: '0.7rem', letterSpacing: '0.22em', animation: 'fadeSlideUpTC 0.35s ease both 0.12s', opacity: 0, animationFillMode: 'forwards' }}>
                  {turnModal.subtitle}
                </p>
                ...
                <p className="text-white font-black leading-none mb-8" style={{ fontFamily: 'Anton', fontSize: 'clamp(2.5rem, 10vw, 3.5rem)', textShadow: `0 4px 20px rgba(0,0,0,0.6), 0 0 40px ${turnModal.accent}44`, animation: 'fadeSlideUpTC 0.35s ease both 0.18s', opacity: 0, animationFillMode: 'forwards' }}>
                  {turnModal.playerName}
                </p>
                <button onClick={() => setTurnModal(null)} className="w-full py-4 font-bold text-sm text-white active:scale-95 transition-transform touch-manipulation" style={{ borderRadius: '16px', background: `linear-gradient(135deg, ${turnModal.accent}33, ${turnModal.accent}18)`, border: `1px solid ${turnModal.accent}55`, boxShadow: `0 4px 16px ${turnModal.accent}22`, letterSpacing: '0.05em', animation: 'fadeSlideUpTC 0.35s ease both 0.25s', opacity: 0, animationFillMode: 'forwards' }}>
                  Entendido ✓
                </button>
              </div>
