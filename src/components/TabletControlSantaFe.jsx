  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(16px)', borderRadius: 24, padding: '40px 24px', border: '2px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 18, textAlign: 'center' }}>
      ...
      <div>
        ...
        <p style={{ margin: 0, fontSize: 15, color: 'rgba(255,255,255,0.65)', lineHeight: 1.5, textShadow: '1px 1px 4px rgba(0,0,0,0.8)' }}><span style={{ color: '#F59E0B', fontWeight: 800 }}>{turnPlayerName}</span>{' está '}{action}</p>
      </div>
    </div>
  );
}
...
export default function TabletControlSantaFe({ sessionId, playerName, playerIndex, matchToken }) {
  ...
  // Auto-redirigir al home cuando el match es cancelado o pospuesto
  useEffect(() => {
  ...
  if (!sessionId) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: SANTAFE_BG, fontFamily: 'Anton, sans-serif', minHeight: '100dvh' }}>
  ...
  if (error && error === 'Sesión no encontrada') {
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: SANTAFE_BG, fontFamily: 'Anton, sans-serif', minHeight: '100dvh' }}>
      ...
      <div className="sticky top-0 z-40 bg-black/95 backdrop-blur-md px-3 pt-3 pb-2 sm:px-4 sm:pt-4 sm:pb-3 border-b border-white/20 shadow-xl">
        ...
        <div className="flex justify-between items-center gap-2">
          <div className="flex items-center gap-2 flex-1">
            ...
            {/* Logo INC centrado en header */}
            <div className="flex-shrink-0 flex flex-col items-center gap-0.5">
              <img src="/images/Smash_Santa_Fe.png" alt="Santa Fe" style={{ width: 32, height: 32, objectFit: 'contain', opacity: 0.9 }} />
              <div className="text-white text-[10px] sm:text-xs font-bold px-1" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.8)' }}>VS</div>
            </div>
      ...
      <div className="p-3 md:p-4 max-w-7xl mx-auto flex flex-col gap-3 md:gap-4 pb-24">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%', maxWidth: 360, alignItems: 'center' }}>
                    <button
                      ...
                      style={{
                        ...
                        background: myChecked ? 'rgba(34,197,94,0.18)' : 'linear-gradient(135deg, rgba(220,38,38,0.25), rgba(245,158,11,0.15))',
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 sm:p-4 shadow-xl border border-white/20 flex-1 flex flex-col overflow-hidden">
            ...
            <div className="text-center mb-2 sm:mb-3 flex-shrink-0">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2">
                ...
                <p className="text-smash-red text-sm sm:text-base font-bold" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.9)' }}>Baneos restantes: {session.bansRemaining}</p>
              </div>
            </div>

            <div className="flex-1 flex flex-col gap-1.5 sm:gap-2 overflow-y-auto pb-2">
              {session.currentGame === 1 ? (
                /* Game 1 - 5 stages fijos de INC: 3+2 */
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2">
                    {GAME1_STAGES_SANTAFE.slice(0, 3).map(s => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-6 gap-1.5 sm:gap-2">
                    ...
                    {GAME1_STAGES_SANTAFE.slice(3, 5).map(s => (
          ...
          <div className="rounded-xl border border-white/20">
                  ...
                  <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                    {playerPickHistory.map((charId) => {
                      ...
                      return (
                        ...
                        >
                          <img src={getStockIconPath(charId, 1) || char.image} alt={char.name} className="w-full h-full object-contain" onError={(e) => { e.target.style.display='none'; }} />
                        </button>
                      );
                    })}
                  </div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 sm:gap-3 p-2 sm:p-4 pb-4 bg-white/10">
              ...
              <button
                ...
                className="aspect-square rounded-lg sm:rounded-xl flex flex-col items-center justify-center active:scale-95 border-2 border-smash-red/70 touch-manipulation"
            ...
            <div className="bg-white/10 rounded-xl sm:rounded-2xl p-2 sm:p-4 border-2 border-white/30">
              <div className="grid grid-cols-3 gap-2 sm:gap-3 items-center">
                <div className="text-center space-y-1 sm:space-y-2">
                  <div className="bg-gradient-to-br from-smash-red/40 to-red-700/40 rounded-lg sm:rounded-xl p-1.5 sm:p-3 border-2 border-red-400/50 shadow-lg">
                    ...
                    <div className="bg-black/30 rounded-md sm:rounded-lg p-1 sm:p-2 border border-white/20">
                      <p className="text-smash-red text-[10px] sm:text-xs font-semibold mb-0.5 sm:mb-1">Personaje</p>
                      <p className="text-white text-xs sm:text-sm font-bold truncate">{getCharacterData(session.player1.character)?.name || 'N/A'}</p>
                    </div>
                  </div>
                  <div className="bg-smash-red/20 rounded-md sm:rounded-lg p-1 sm:p-2 border-2 border-smash-red/50">
                    <p className="text-white/70 text-[10px] sm:text-xs font-semibold">Score</p>
                    <p className="text-smash-red text-xl sm:text-2xl font-black">{session.player1.score}</p>
                  </div>
                </div>
                ...
                <div className="text-center space-y-1 sm:space-y-2">
                  ...
                  <div className="bg-smash-red/20 rounded-md sm:rounded-lg p-1 sm:p-2 border-2 border-smash-red/50">
                    <p className="text-white/70 text-[10px] sm:text-xs font-semibold">Score</p>
                    <p className="text-smash-red text-xl sm:text-2xl font-black">{session.player2.score}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              {session.winnerProposal && myPlayer && session.winnerProposal.proposedBy !== myPlayer ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="py-4 sm:py-5 rounded-xl font-black text-base sm:text-lg text-white active:scale-95 transition-all shadow-2xl border-2 border-red-400/60 touch-manipulation"
                  ...
                  <button
                    ...
                    className="py-4 sm:py-5 rounded-xl font-black text-base sm:text-lg text-white active:scale-95 transition-all shadow-2xl border-2 border-blue-400/60 touch-manipulation"
        ...
        {session.phase === 'FINISHED' && !finishedDismissed && (
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 shadow-2xl border border-white/20 text-center flex-1 flex flex-col justify-center">
            ...
            <div className="bg-gradient-to-r from-smash-red/20 via-red-600/20 to-smash-red/20 rounded-xl p-4 mb-4 border-2 border-smash-red/50">
              <p className="text-white/70 text-sm mb-1">Ganador</p>
              <p className="text-smash-red text-3xl font-bold">
                ...
                <div className="mb-4 flex justify-center">
                  {pendingAction.type === 'character' && pendingAction.skin && pendingAction.characterName ? (
                    <div className="w-16 h-16 bg-white/10 rounded-xl border-4 border-smash-red p-1 flex items-center justify-center">
                      <img src={getStockIconPath(pendingAction.characterId, pendingAction.skin) || pendingAction.characterImage} alt={pendingAction.characterName} className="w-full h-full object-contain" onError={(e) => { e.target.style.display='none'; }} />
                    </div>
          ...
          >
            <div
              ...
              style={{
                ...
                animation: 'modalPopInc 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) both',
              }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="absolute top-0 left-0 right-0" style={{ height: '3px', background: `linear-gradient(90deg, transparent, ${turnModal.accent}, transparent)`, animation: 'accentPulseInc 2s ease-in-out infinite' }} />
              <div className="px-8 pt-10 pb-8">
                <div className="mx-auto mb-5 flex items-center justify-center text-5xl" style={{ width: 90, height: 90, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', border: `2px solid ${turnModal.accent}66`, boxShadow: `0 0 24px ${turnModal.accent}44`, animation: 'fadeSlideUpInc 0.35s ease both 0.05s' }}>
                  {turnModal.icon}
                </div>
                <p className="font-bold uppercase" style={{ color: turnModal.accent, fontSize: '0.7rem', letterSpacing: '0.22em', animation: 'fadeSlideUpInc 0.35s ease both 0.12s', opacity: 0, animationFillMode: 'forwards' }}>
                  {turnModal.subtitle}
                </p>
                ...
                <p className="text-white font-black leading-none mb-8" style={{ fontFamily: 'Anton', fontSize: 'clamp(2.5rem, 10vw, 3.5rem)', textShadow: `0 4px 20px rgba(0,0,0,0.6), 0 0 40px ${turnModal.accent}44`, animation: 'fadeSlideUpInc 0.35s ease both 0.18s', opacity: 0, animationFillMode: 'forwards' }}>
                  {turnModal.playerName}
                </p>
                <button onClick={() => setTurnModal(null)} className="w-full py-4 font-bold text-sm text-white active:scale-95 transition-transform touch-manipulation" style={{ borderRadius: '16px', background: `linear-gradient(135deg, ${turnModal.accent}33, ${turnModal.accent}18)`, border: `1px solid ${turnModal.accent}55`, boxShadow: `0 4px 16px ${turnModal.accent}22`, letterSpacing: '0.05em', animation: 'fadeSlideUpInc 0.35s ease both 0.25s', opacity: 0, animationFillMode: 'forwards' }}>
                  Entendido ✓
                </button>
              </div>
