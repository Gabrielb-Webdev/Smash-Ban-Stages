...
// ───────────────────────────────────────────────────────────────
export default function TabletControlMendoza({ sessionId, playerName, playerIndex }) {
          ...
          <div className="bg-white/10 rounded-xl p-2 sm:p-4 border-2 border-white/30 flex-1 flex flex-col justify-center">
            ...
            <div className="mt-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setPendingAction({ type: 'winner', winner: 'player1', winnerName: session.player1.name })} className="py-4 rounded-xl border-2 font-black text-sm sm:text-base active:scale-95 touch-manipulation transition-all" style={{ background: `linear-gradient(135deg, ${THEME.colors.primary}4D, ${THEME.colors.primary}33)`, borderColor: `${THEME.colors.primary}80`, color: '#fff', fontFamily: 'inherit', cursor: 'pointer' }}>
                  🔴 {session.player1.name}
                </button>
                <button onClick={() => setPendingAction({ type: 'winner', winner: 'player2', winnerName: session.player2.name })} className="py-4 rounded-xl border-2 font-black text-sm sm:text-base active:scale-95 touch-manipulation transition-all" style={{ background: `linear-gradient(135deg, ${THEME.colors.secondary}4D, ${THEME.colors.secondary}33)`, borderColor: `${THEME.colors.secondary}80`, color: '#fff', fontFamily: 'inherit', cursor: 'pointer' }}>
            ...
            <div className="rounded-2xl p-4 shadow-2xl border-4 max-w-sm w-full" style={{ background: `linear-gradient(135deg, ${THEME.colors.primary}, ${THEME.colors.secondary})`, borderColor: THEME.colors.accent }}>
              <div className="text-center mb-6">
                <div className="mb-4 flex justify-center">
                  {pendingAction.type === 'character' && pendingAction.characterImage ? (
                    ...
                    <div className="text-4xl">
                      ...
                      {pendingAction.type === 'character' && '👤'}                      {pendingAction.type === 'winner' && '🏆'}                    </div>
                  )}
                </div>
                ...
                <div className="bg-white/10 rounded-xl p-2 mt-2">
                  ...
                  {pendingAction.type === 'winner' && <p className="text-white text-base"><span className="font-bold" style={{ color: THEME.colors.accent }}>{pendingAction.winnerName}</span> ganó el Game {session.currentGame}</p>}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
