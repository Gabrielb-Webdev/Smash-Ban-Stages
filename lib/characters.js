...
/** Genera las rutas de alts para un personaje (alt03 a alt08) */
export function charAltPaths(charId) {
  const folder = CHARACTER_ALT_FOLDERS[charId];
  if (!folder) return [];
  return [3,4,5,6,7,8].map(n => `/images/Render%20SSBU/alts/${folder}/${folder}_alt${String(n).padStart(2,'0')}.png`);
}
