...
import {
  ...
  isInPlacement, leaderboardScore,
} from '../ranks.js';
...
describe('leaderboardScore', () => {
  it('score = rankIndex * RANK_MAX + rankPoints', () => {
    expect(leaderboardScore({ rankIndex: 12, rankPoints: 50 })).toBe(12 * 100 + 50);
  });

  it('SMASHer tiene score alto', () => {
    const s = leaderboardScore({ rankIndex: 28, rankPoints: 0 });
    expect(s).toBe(2800);
  });
