...
/** Notificaciones broadcast (globales para todos): lista JSON */
export const broadcastNotifsKey = 'notifications:broadcast';
...
/** Lista de torneos del ranking comunitario: JSON array */
export const crTournamentsKey = (community, year) => `cr:${community}:${year}:tournaments`;

/** Overrides manuales de personaje por jugador: objeto JSON { "nombre_lower": "charId" } */
export const crCharOverrideKey = (community, year) => `cr:char-override:${community}:${year}`;

/** Config de comunidades visibles en el tab Rankings del home */
export const rankingConfigKey = 'ranking:home-communities';

/** Presencia online de un usuario: string con TTL (60s) */
export const presenceKey = (userId) => `presence:${userId}`;

/** Estado manual del usuario: 'online' | 'away' | 'dnd' | 'invisible' */
export const userStatusKey = (userId) => `user:status:${userId}`;

/** Historial de cambios de rango de un jugador: lista JSON (max 100) */
export const rankHistoryKey = (userId) => `ranked:history:${userId}`;

/** Registro de matchups recientes entre dos usuarios (anti queue-sniping): hash con TTL diario */
export const mmMatchupKey = (platform) => `mm:matchups:${platform}`;

/** MMR snapshot de jugadores activos (últimos 30 días) para cálculo de SMASHer threshold */
export const smasherPoolKey = (platform) => `ranked:smasher:pool:${platform}`;

/** Leaderboard SMASHer — sorted set con score = MMR (solo jugadores en tier SMASHer) */
export const smasherBoardKey = (platform) => `ranked:smasher:board:${platform}`;

/** Cola de matchmaking CASUAL por plataforma: lista JSON */
export const casualQueueKey = (platform) => `mm:casual:queue:${platform}`;

/** Room de partida casual activa: objeto JSON */
export const casualMatchKey = (matchId) => `mm:casual:match:${matchId}`;

// ── Tickets / Disputas ──────────────────────────────────────────

/** Ticket de disputa: objeto JSON */
export const ticketKey = (ticketId) => `ticket:${ticketId}`;

/** Lista de IDs de tickets abiertos (orden inserción): lista */
export const ticketsOpenKey = 'tickets:open';

/** Lista de IDs de tickets cerrados (orden inserción): lista */
export const ticketsClosedKey = 'tickets:closed';

/** Tickets de un usuario: lista de ticketIds */
export const ticketsByUserKey = (userId) => `tickets:by-user:${userId}`;

/** Tickets de un match: lista de ticketIds */
export const ticketsByMatchKey = (matchId) => `tickets:by-match:${matchId}`;

/** Imagen de evidencia (base64) con TTL 90 días */
export const ticketImageKey = (ticketId) => `ticket:img:${ticketId}`;

/** Set de slugs/userIds con rol de soporte */
export const supportUsersKey = 'support:users';

// ── Ranked Offline (torneos presenciales) ───────────────────────

/** Sesión de Ranked Offline activa: objeto JSON */
export const offlineSessionKey = () => 'offline:session';

/** Cola de jugadores esperando ser asignados: array JSON */
export const offlineQueueKey = () => 'offline:queue';

/** Partidas activas/terminadas de la sesión: array JSON */
export const offlineMatchesKey = () => 'offline:matches';

/** Resultado de la última partida offline de un jugador (TTL 600s) */
