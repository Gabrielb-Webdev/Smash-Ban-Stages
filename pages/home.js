        ...
        {!is2v2 && matchStatus !== 'disputed' && (
          showForfeitConfirm ? (
            <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 16, padding: '14px 16px', textAlign: 'center' }}>
              <p style={{ margin: '0 0 4px', fontSize: 14, fontWeight: 800, color: '#EF4444' }}>??? ¿Confirmás que querés rendirte?</p>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={forfeit} disabled={forfeitLoading} style={{ flex: 1, padding: '10px', borderRadius: 12, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.12)', color: '#EF4444', fontWeight: 800, fontSize: 13, cursor: forfeitLoading ? 'not-allowed' : 'pointer' }}>{forfeitLoading ? '? Procesando…' : '??? Sí, rendirme'}</button>
              </div>
            </div>
          ) : (
    ...
    <div style={{ padding: '24px 18px' }}>
      ...
      <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {matchTypeMode === 'casual' ? (
          ...
          [['??','Invitá a un amigo','Desde tu perfil, tocá ?? y usá el botón 2v2'],['?','Tu amigo acepta','Cuando acepte, el party estará listo'],['??','Buscar rivales','Entrá a la cola como equipo de 2'],['??','Los 4 aceptan','Ambos equipos confirman la partida'],['??','Reportá el resultado','El equipo ganador reporta y sube de rango']].map(([icon,t,d])=>(
            <div key={t} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '4px 0' }}>
              <span style={{ fontSize: 16, flexShrink: 0, marginTop: 1 }}>{icon}</span>
              <div>
                <p style={{ margin: '0 0 1px', fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.8)' }}>{t}</p>
